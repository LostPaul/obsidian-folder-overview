...
function addExcludePatternListItem(settings, containerEl, pattern) {
	...
	if (import_obsidian8.Platform.isDesktop || import_obsidian8.Platform.isTablet) {
		setting.addButton((cb) => {
...
var WhitelistFolderSettings = class extends import_obsidian9.Modal {
	...
	display() {
		...
		new import_obsidian9.Setting(contentEl).setName('Include subfolders').setDesc('Choose if the subfolders of the folder should also be whitelisted').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.subFolders).onChange(async (value) => {
			this.whitelistedFolder.subFolders = value;
			await this.plugin.saveSettings(true);
		}));
		new import_obsidian9.Setting(contentEl).setName('Enable folder name sync').setDesc('Choose if the name of a folder note should be renamed when the folder name is changed').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.enableSync).onChange(async (value) => {
			this.whitelistedFolder.enableSync = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian9.Setting(contentEl).setName('Show folder in folder overview').setDesc('Choose if the folder should be shown in the folder overview').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.showInFolderOverview).onChange(async (value) => {
			this.whitelistedFolder.showInFolderOverview = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian9.Setting(contentEl).setName('Allow auto creation of folder notes in this folder').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.enableAutoCreate).onChange(async (value) => {
			this.whitelistedFolder.enableAutoCreate = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian9.Setting(contentEl).setName('Open folder note when clicking on the folder').setDesc('Choose if the folder note should be opened when the folder is opened').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.enableFolderNote).onChange(async (value) => {
			this.whitelistedFolder.enableFolderNote = value;
			await this.plugin.saveSettings(true);
			this.display();
		}));
		if (this.whitelistedFolder.enableFolderNote) {
			new import_obsidian9.Setting(contentEl).setName('Collapse folder when opening folder note').setDesc('Choose if the folder should be collapsed when the folder note is opened').addToggle((toggle) => toggle.setValue(this.whitelistedFolder.enableCollapsing).onChange(async (value) => {
...
var WhitelistPatternSettings = class extends import_obsidian10.Modal {
	constructor(app2, plugin, pattern) {
		super(app2);
		this.plugin = plugin;
		this.app = app2;
	...
	display() {
		...
		new import_obsidian10.Setting(contentEl).setName('Enable folder name sync').setDesc('Choose if the name of a folder note should be renamed when the folder name is changed').addToggle((toggle) => toggle.setValue(this.pattern.enableSync).onChange(async (value) => {
			this.pattern.enableSync = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian10.Setting(contentEl).setName('Allow auto creation of folder notes in this folder').addToggle((toggle) => toggle.setValue(this.pattern.enableAutoCreate).onChange(async (value) => {
			this.pattern.enableAutoCreate = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian10.Setting(contentEl).setName('Show folder in folder overview').setDesc('Choose if the folder should be shown in the folder overview').addToggle((toggle) => toggle.setValue(this.pattern.showInFolderOverview).onChange(async (value) => {
			this.pattern.showInFolderOverview = value;
			await this.plugin.saveSettings();
		}));
		new import_obsidian10.Setting(contentEl).setName('Open folder note when clicking on the folder').setDesc('Choose if the folder note should be opened when you click on the folder').addToggle((toggle) => toggle.setValue(this.pattern.enableFolderNote).onChange(async (value) => {
...
function addWhitelistFolderListItem(settings, containerEl, whitelistedFolder) {
	...
	const SearchComponent = new import_obsidian12.Setting(inputContainer);
	...
	new import_obsidian12.ButtonComponent(buttonContainer).setIcon('edit').setTooltip('Edit folder note').onClick(() => {
		new WhitelistFolderSettings(plugin.app, plugin, whitelistedFolder).open();
	});
	new import_obsidian12.ButtonComponent(buttonContainer).setIcon('up-chevron-glyph').setTooltip('Move up').onClick(() => {
	...
	new import_obsidian12.ButtonComponent(buttonContainer).setIcon('down-chevron-glyph').setTooltip('Move down').onClick(() => {
	...
	new import_obsidian12.ButtonComponent(buttonContainer).setIcon('trash-2').setTooltip('Delete excluded folder').onClick(() => {
		deleteWhitelistedFolder(plugin, whitelistedFolder);
		setting.clear();
		setting.settingEl.remove();
	});
...
function addExcludeFolderListItem(settings, containerEl, excludedFolder) {
	...
	if (import_obsidian13.Platform.isDesktop || import_obsidian13.Platform.isTablet) {
		setting.addButton((cb) => {
			cb.setIcon('up-chevron-glyph');
			cb.setTooltip('Move up');
			cb.onClick(() => {
...
var AskForExtensionModal = class extends import_obsidian14.FuzzySuggestModal {
...
async function openFolderNote(plugin, file, evt) {
	...
	if (((_a = plugin.app.workspace.getActiveFile()) == null ? void 0 : _a.path) === path && !(import_obsidian16.Keymap.isModEvent(evt) == 'tab')) {
		return;
	}
	const leaf = plugin.app.workspace.getLeaf(import_obsidian16.Keymap.isModEvent(evt) || plugin.settings.openInNewTab);
...
var AddSupportedFileModal = class extends import_obsidian17.Modal {
	...
	async onClose() {
		...
		if (this.name === '') {
			...
			return new import_obsidian17.Notice('This extension is already supported');
		} else {
...
var TemplateSuggest = class extends import_obsidian20.AbstractInputSuggest {
	...
	renderSuggestion(file, el) {
		...
		const { templateFolder, templaterPlugin } = getTemplatePlugins(this.app);
		if ((!templateFolder || templateFolder.trim() === '') && !templaterPlugin) {
			el.setText(`${(_a = file.parent) == null ? void 0 : _a.path}/${file.name.replace('.md', '')}`);
		} else {
...
async function renderGeneral(settingsTab) {
	...
	const nameSetting = new import_obsidian23.Setting(containerEl).setName('Folder note name').setDesc('{{folder_name}} will be replaced with the name of the folder').addText((text) => text.setValue(settingsTab.plugin.settings.folderNoteName).onChange(async (value) => {
		if (value.trim() === '') {
			return;
		}
	...
	if (settingsTab.plugin.settings.folderNoteName !== '{{folder_name}}') {
		new import_obsidian23.Setting(containerEl).setName('Use folder name instead of folder note name in the tab title').setDesc('When you\'re using a folder note name like "folder note" and have multiple folder notes open you can\'t separate them anymore by their name. This setting uses the folder name instead and allows you to indentify the different files.').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.tabManagerEnabled).onChange(async (value) => {
			if (!value) {
	...
	new import_obsidian23.Setting(containerEl).setName('Default folder note type for new folder notes').setDesc('Choose the default file type for new folder notes. (canvas, markdown, ...)').addDropdown((dropdown) => {
	...
	const templateSetting = new import_obsidian23.Setting(containerEl).setDesc('Requires templates/templater plugin to be enabled').setName('Template path').addSearch((cb) => {
		var _a;
	...
	if (settingsTab.plugin.settings.storageLocation === 'parentFolder') {
		...
		new import_obsidian23.Setting(containerEl).setName('Move folder notes when moving the folder').setDesc('Move the folder note when moving the folder').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.syncMove).onChange(async (value) => {
	...
	new import_obsidian23.Setting(containerEl).setName('Enable front matter title plugin integration').setDesc('Automatically rename a folder name when the folder note is renamed').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.frontMatterTitle.enabled).onChange(async (value) => {
		var _a;
	...
	new import_obsidian23.Setting(containerEl).setName('Create folder note for every folder').setDesc('Create a folder note for every folder in the vault').addButton((cb) => {
		cb.setIcon('plus');
	...
	new import_obsidian23.Setting(containerEl).setName('Automatically create folder notes').setDesc('Automatically create a folder note when a new folder is created').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.autoCreate).onChange(async (value) => {
		settingsTab.plugin.settings.autoCreate = value;
		await settingsTab.plugin.saveSettings();
		settingsTab.display();
	}));
	if (settingsTab.plugin.settings.autoCreate) {
		new import_obsidian23.Setting(containerEl).setName('Open folder note after creating').setDesc('Automatically open the folder note after automatically creating it').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.autoCreateFocusFiles).onChange(async (value) => {
			settingsTab.plugin.settings.autoCreateFocusFiles = value;
			await settingsTab.plugin.saveSettings();
			settingsTab.display();
		}));
		new import_obsidian23.Setting(containerEl).setName('Auto create folder note for attachment folder').setDesc('Automatically create a folder note for the attachment folder when you attach a file to a note and the attachment folder gets created').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.autoCreateForAttachmentFolder).onChange(async (value) => {
			settingsTab.plugin.settings.autoCreateForAttachmentFolder = value;
			await settingsTab.plugin.saveSettings();
			settingsTab.display();
		}));
	}
	new import_obsidian23.Setting(containerEl).setName('Automatically create folder note when you create a note').setDesc('Automatically create a folder note when a note is created. It has to be a file type that you selected in the supported file types').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.autoCreateForFiles).onChange(async (value) => {
...
async function renderFileExplorer(settingsTab) {
	const containerEl = settingsTab.settingsPage;
	new import_obsidian24.Setting(containerEl).setName('Hide folder note').setDesc('Hide the folder note in the file explorer').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.hideFolderNote).onChange(async (value) => {
	...
	new import_obsidian24.Setting(containerEl).setName('Only open folder notes through the name').setDesc('Only open folder notes in the file explorer by clicking on the folder name').addToggle((toggle) => toggle.setValue(!settingsTab.plugin.settings.stopWhitespaceCollapsing).onChange(async (value) => {
		if (!value) {
	...
	if (settingsTab.plugin.settings.frontMatterTitle.enabled) {
		new import_obsidian24.Setting(containerEl).setName('Change folder name in the file explorer').setDesc('Automatically rename a folder name in the file explorer when the folder note is renamed').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.frontMatterTitle.explorer).onChange(async (value) => {
	...
	new import_obsidian24.Setting(containerEl).setName('Highlight folder in the file explorer').setDesc('Highlight the folder name in the file explorer when you click on a folder that has a folder note').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.highlightFolder).onChange(async (value) => {
	...
	if (settingsTab.plugin.settings.hideCollapsingIcon) {
		new import_obsidian24.Setting(containerEl).setName('Hide collapse icon also when the attachment folder is in the same folder').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.ignoreAttachmentFolder).onChange(async (value) => {
	...
	new import_obsidian24.Setting(containerEl).setName('Bold the name of folder notes').setDesc('Make the folder name bold in the file explorer').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.boldName).onChange(async (value) => {
	...
	new import_obsidian24.Setting(containerEl).setName('Cursive the name of folder notes').setDesc('Make the folder name cursive in the file explorer').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.cursiveName).onChange(async (value) => {
...
async function renderPath(settingsTab) {
	...
	new import_obsidian25.Setting(containerEl).setName('Change folder name in the path').setDesc('Automatically rename a folder name in the path above a note when the folder note is renamed').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.frontMatterTitle.path).onChange(async (value) => {
	...
	new import_obsidian25.Setting(containerEl).setName('Bold folders in the path').setDesc('Make the folder name bold in the path above a note').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.boldNameInPath).onChange(async (value) => {
	...
	new import_obsidian25.Setting(containerEl).setName('Cursive the name of folder notes in the path').setDesc('Make the folder name cursive in the path above a note').addToggle((toggle) => toggle.setValue(settingsTab.plugin.settings.cursiveNameInPath).onChange(async (value) => {
...
// src/ExcludeFolders/modals/WhitelistedFoldersSettings.ts
var WhitelistedFoldersSettings = class extends import_obsidian26.Modal {
	...
	onOpen() {
		...
		new import_obsidian26.Setting(contentEl).setName('Add whitelisted folder').setClass('add-exclude-folder-item').addButton((cb) => {
			cb.setIcon('plus');
...
async function renderExcludeFolders(settingsTab) {
	const containerEl = settingsTab.settingsPage;
	const manageExcluded = new import_obsidian27.Setting(containerEl).setHeading().setClass('fn-excluded-folder-heading').setName('Manage excluded folders');
	...
	new import_obsidian27.Setting(containerEl).setName('Whitelisted folders').setDesc('Folders that override the excluded folders/patterns').addButton((cb) => {
	...
	new import_obsidian27.Setting(containerEl).setName('Exclude folder default settings').addButton((cb) => {
		cb.setButtonText('Manage');
		cb.setCta();
		cb.onClick(async () => {
	...
	new import_obsidian27.Setting(containerEl).setName('Exclude pattern default settings').addButton((cb) => {
		cb.setButtonText('Manage');
		cb.setCta();
		cb.onClick(async () => {
	...
	new import_obsidian27.Setting(containerEl).setName('Add excluded folder').setClass('add-exclude-folder-item').addButton((cb) => {
		cb.setIcon('plus');
		cb.setClass('add-exclude-folder');
...
var SettingsTab = class extends import_obsidian28.PluginSettingTab {
	...
	renameFolderNotes() {
		var _a, _b, _c;
		new import_obsidian28.Notice('Starting to update folder notes...');
		...
		this.plugin.saveSettings();
		new import_obsidian28.Notice('Finished updating folder notes');
	}
	switchStorageLocation(oldMethod) {
		new import_obsidian28.Notice('Starting to switch storage location...');
		...
		new import_obsidian28.Notice('Finished switching storage location');
	}
	onClose() {
		...
		this.plugin.addCommand({
			...
			callback: async () => {
				...
				if (this.plugin.app.vault.getAbstractFileByPath(newPath)) {
					return new import_obsidian29.Notice('Folder already exists');
				}
				...
				if (text && text.trim() !== '') {
					...
					for (const char of blacklist) {
						if (text.includes(char)) {
							new import_obsidian29.Notice('File name cannot contain any of the following characters: * " \\ / < > : | ?');
							return false;
						}
					}
					if (text.endsWith('.')) {
						new import_obsidian29.Notice('File name cannot end with a dot');
						return;
					}
					let folder;
					...
					if (folderPath === '') {
						...
						if (folder instanceof import_obsidian29.TFolder) {
							new import_obsidian29.Notice('Folder note already exists');
							return false;
						} else {
						...
						if (this.plugin.settings.storageLocation === 'parentFolder') {
							if (this.app.vault.getAbstractFileByPath(folderPath + '/' + text + this.plugin.settings.folderNoteType)) {
								new import_obsidian29.Notice('File already exists');
								return false;
							}
						}
	...
	fileCommands() {
		this.plugin.registerEvent(this.app.workspace.on('file-menu', async (menu, file) => {
			...
			if (file instanceof import_obsidian29.TFile) {
				...
				} else if (file.parent instanceof import_obsidian29.TFolder) {
					folder = file.parent;
				}
			}
			menu.addItem(async (item) => {
				if (import_obsidian29.Platform.isDesktop && !import_obsidian29.Platform.isTablet && this.plugin.settings.useSubmenus) {
					item.setTitle('Folder Note Commands').setIcon('folder-edit');
				}
				let subMenu;
				if (!import_obsidian29.Platform.isDesktopApp || !import_obsidian29.Platform.isDesktop || import_obsidian29.Platform.isTablet || !this.plugin.settings.useSubmenus) {
					subMenu = menu;
				...
				if (file instanceof import_obsidian29.TFile) {
					subMenu.addItem((item2) => {
						item2.setTitle('Create folder note').setIcon('edit').onClick(async () => {
							...
							if (!(newFolder instanceof import_obsidian29.TFolder))
								return;
					...
					subMenu.addItem((item2) => {
						item2.setTitle(`Turn into folder note for ${folder == null ? void 0 : folder.name}`).setIcon('edit').onClick(() => {
							if (!folder || !(folder instanceof import_obsidian29.TFolder))
								return;
				...
				if (excludedFolder && !excludedFolder.hideNote) {
					subMenu.addItem((item2) => {
						item2.setTitle('Remove folder from excluded folders').setIcon('trash').onClick(() => {
							...
							new import_obsidian29.Notice('Successfully removed folder from excluded folders');
						});
					});
					return;
				}
				...
				subMenu.addItem((item2) => {
					item2.setTitle('Exclude folder from folder notes').setIcon('x-circle').onClick(() => {
						...
						this.plugin.saveSettings(true);
						new import_obsidian29.Notice('Successfully excluded folder from folder notes');
					});
				});
				if (!(file instanceof import_obsidian29.TFolder))
					return;
				const folderNote = getFolderNote(this.plugin, file.path);
				if (folderNote instanceof import_obsidian29.TFile && !detachedExcludedFolder) {
					subMenu.addItem((item2) => {
			...
			menu.addItem((item) => {
				item.setTitle('Create folder note').setIcon('edit').onClick(() => {
					...
					if (folderPath === '') {
						folder = this.plugin.app.vault.getAbstractFileByPath(text);
						if (folder instanceof import_obsidian29.TFolder) {
							return new import_obsidian29.Notice('Folder note already exists');
						} else {
							this.plugin.app.vault.createFolder(text);
							createFolderNote(this.plugin, text, false);
						}
						...
						if (this.plugin.settings.storageLocation === 'parentFolder') {
							if (this.app.vault.getAbstractFileByPath(folderPath + '/' + fileName + this.plugin.settings.folderNoteType)) {
								return new import_obsidian29.Notice('File already exists');
							}
						}
						this.plugin.app.vault.createFolder(folderPath + '/' + text);
						createFolderNote(this.plugin, folderPath + '/' + text, false);
...
async function handleViewHeaderClick(event, plugin) {
	...
	if (folderNote) {
		await openFolderNote(plugin, folderNote, event).then(async () => {
			...
			if (fileExplorerPlugin && import_obsidian30.Platform.isMobile && plugin.settings.openSidebar.mobile) {
				setTimeout(() => {
					fileExplorerPlugin.revealInFolder(folderNote);
				}, 200);
			} else if (fileExplorerPlugin && import_obsidian30.Platform.isDesktop && plugin.settings.openSidebar.desktop) {
				fileExplorerPlugin.revealInFolder(folderNote);
			}
		});
		return;
	} else if (event.altKey || import_obsidian30.Keymap.isModEvent(event) === 'tab') {
		if (plugin.settings.altKey && event.altKey || plugin.settings.ctrlKey && import_obsidian30.Keymap.isModEvent(event) === 'tab') {
...
async function handleFolderClick(event, plugin) {
	...
	if (folderNote) {
		...
		} else if (plugin.settings.openWithCtrl && import_obsidian30.Keymap.isModEvent(event) === 'tab') {
			return openFolderNote(plugin, folderNote, event);
...
async function initializeFolderTitle(folderTitle, plugin) {
	...
	if (folderTitle.onclick)
		return;
	if (import_obsidian31.Platform.isMobile && plugin.settings.disableOpenFolderNoteOnClick)
		return;
	...
	plugin.registerDomEvent(folderTitle, 'pointerover', (event) => {
		...
		if (!import_obsidian31.Keymap.isModEvent(event))
			return;
		if (!(event.target instanceof HTMLElement))
			return;
...
function handleRename(file, oldPath, plugin) {
	...
	if (oldFolder instanceof import_obsidian32.TFolder) {
	...
	} else if (file instanceof import_obsidian32.TFile) {
		if (isRename) {
...
async function renameFolderOnFileRename(file, oldPath, oldFolder, plugin) {
	...
	if (plugin.app.vault.getAbstractFileByPath(newFolderPath)) {
		...
		return new import_obsidian32.Notice('A folder with the same name already exists');
	}
...
async function handleCreate(file, plugin) {
	...
	} else if (file instanceof import_obsidian33.TFolder && plugin.settings.autoCreate) {
		handleFolderCreation(file, plugin);
	}
}
async function handleFileCreation(file, plugin) {
	const folder = getFolder2(plugin, file);
	if (!(folder instanceof import_obsidian33.TFolder) && plugin.settings.autoCreateForFiles) {
		if (!file.parent) {
			return;
		}
		const newFolder = await plugin.app.fileManager.createNewFolder(file.parent);
		turnIntoFolderNote(plugin, file, newFolder);
	} else if (folder instanceof import_obsidian33.TFolder) {
...
var TabManager = class {
	...
	resetTabs() {
		...
		this.app.workspace.iterateAllLeaves((leaf) => {
			var _a;
			if (!(leaf.view instanceof import_obsidian34.EditableFileView))
				return;
		...
		editorCheckCallback: (checking, editor) => {
			...
			if (lineText.trim() === '' || lineText.trim() === '>') {
				if (!checking) {
					...
					const yaml = (0, import_obsidian36.stringifyYaml)(json);
					if (lineText.trim() === '') {
...
// src/main.ts
var FolderNotesPlugin = class extends import_obsidian37.Plugin {
	constructor() {
		super(...arguments);
	...
	async onload() {
		...
		if ((0, import_obsidian37.requireApiVersion)('1.7.2')) {
		...
		this.registerDomEvent(window, 'keydown', (event) => {
			...
			if (!hoveredElement)
				return;
			if (!import_obsidian37.Keymap.isModEvent(event))
				return;
	...
	onLayoutReady() {
		...
		editMode.clipboardManager.constructor.prototype.handleDragOver = function(evt, ...args) {
			...
			if (draggable && draggable.file instanceof import_obsidian37.TFolder && getFolderNote(plugin, draggable.file.path)) {
		...
		editMode.clipboardManager.constructor.prototype.handleDrop = function(evt, ...args) {
			...
			if (draggable && draggable.file instanceof import_obsidian37.TFolder && getFolderNote(plugin, draggable.file.path)) {
				...
				if ((draggable == null ? void 0 : draggable.type) === 'folder' && draggable.file instanceof import_obsidian37.TFolder && folderNote) {
	...
	handleOverviewBlock(source, el, ctx) {
		const observer = new MutationObserver(() => {
			...
			if (editButton) {
				editButton.addEventListener('click', (e) => {
					...
					e.preventDefault();
					e.stopPropagation();
					new FolderOverviewSettings(this.app, this, (0, import_obsidian37.parseYaml)(source), ctx, el, this.settings.defaultOverview).open();
				}, { capture: true });
			}
		});
		observer.observe(el, {
		...
		try {
			if (this.app.workspace.layoutReady) {
				...
				folderOverview.create(this, (0, import_obsidian37.parseYaml)(source), el, ctx);
			} else {
			...
			new import_obsidian37.Notice('Error creating folder overview (folder notes plugin) - check console for more details');
			console.error(e);
		}
	}
	...
	isEmptyFolderNoteFolder(folder) {
		...
		if (folder.children.length == threshold) {
			...
			if (attachmentsAreInRootFolder) {
				...
				if (attachmentFolder instanceof import_obsidian37.TFolder && folder.children.length <= threshold + 1) {
					if (!folder.collapsed) {
	...
	updateBreadcrumbs(remove) {
		...
		const files = this.app.vault.getAllLoadedFiles().filter((file) => file instanceof import_obsidian37.TFolder);
		viewHeaderItems.forEach((item) => {
...
// src/obsidian-folder-overview/src/suggesters/FolderSuggester.ts
var FolderSuggest2 = class extends import_obsidian38.AbstractInputSuggest {
	constructor(inputEl, plugin, whitelistSuggester, folder) {
		super(plugin.app, inputEl);
		this.inputEl = inputEl;
		this.whitelistSuggester = whitelistSuggester;
	...
	getSuggestions(input_str) {
		...
		files.forEach((folder) => {
			if (folder instanceof import_obsidian38.TFolder && folder.path.toLowerCase().contains(lower_input_str) && (this.plugin instanceof FolderNotesPlugin ? !this.plugin.settings.excludeFolders.find((f) => f.path === folder.path) || this.whitelistSuggester : true)) {
				folders.push(folder);
			}
		});
		return folders;
...
var ListComponent2 = class {
	...
	addElement(value) {
		this.listEl.createSpan('setting-hotkey', (span) => {
			...
			const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="svg-icon lucide-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';
			const svgElement = removeSpan.createEl('span', { cls: 'ofn-list-item-remove-icon' });
			(0, import_obsidian39.setIcon)(svgElement, 'x');
			removeSpan.onClickEvent((e) => {
				this.removeValue(value);
				span.remove();
			});
	...
	addResetButton() {
		const resetButton = this.controlEl.createEl('span', { cls: 'clickable-icon setting-restore-hotkey-button' });
		const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="svg-icon lucide-rotate-ccw"><path d="M3 2v6h6"></path><path d="M3 13a9 9 0 1 0 3-7.7L3 8"></path></svg>';
		(0, import_obsidian39.setIcon)(resetButton, 'rotate-ccw');
		resetButton.onClickEvent((e) => {
			this.setValues(this.defaultValues);
		});
		return this;
...
async function createOverviewSettings(contentEl, yaml, plugin, defaultSettings, display, el, ctx, file, settingsTab, modal, changedSection) {
	...
	createOrReplaceSetting(contentEl, 'auto-sync', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Auto sync').setDesc('Choose if the overview should automatically update when you delete, create or rename a file/folder').addToggle((toggle) => toggle.setValue(yaml.autoSync).onChange(async (value) => {
			yaml.autoSync = value;
	...
	createOrReplaceSetting(contentEl, 'allow-drag-and-drop', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Allow drag and drop').setDesc('Choose if you want to allow drag and drop in the overview').addToggle((toggle) => toggle.setValue(yaml.allowDragAndDrop).onChange(async (value) => {
	...
	createOrReplaceSetting(contentEl, 'showTitle', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Show the title').setDesc('Choose if the title should be shown').addToggle((toggle) => toggle.setValue(yaml.showTitle).onChange(async (value) => {
	...
	createOrReplaceSetting(contentEl, 'title-container-fn', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Title').setDesc('Choose the title of the folder overview').addText((text) => text.setValue((yaml == null ? void 0 : yaml.title) || '{{folderName}} overview').onChange(async (value) => {
			yaml.title = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
		}));
	});
	createOrReplaceSetting(contentEl, 'folder-path', changedSection, (settingEl) => {
		const folderPathSetting = new import_obsidian40.Setting(settingEl).setName('Folder path for the overview').setDesc('Choose the folder path for the overview').addSearch((search) => {
			...
			search.setPlaceholder('Folder path').setValue((yaml == null ? void 0 : yaml.folderPath) || '').onChange(async (value) => {
				if (value.trim() !== '') {
					value = (0, import_obsidian40.normalizePath)(value);
				}
				if (!(plugin.app.vault.getAbstractFileByPath(value) instanceof import_obsidian40.TFolder) && value !== '')
					return;
	...
	createOrReplaceSetting(contentEl, 'overview-style', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Overview style').setDesc('Choose the style of the overview (grid style soon)').addDropdown((dropdown) => dropdown.addOption('list', 'List').addOption('explorer', 'Explorer').setValue((yaml == null ? void 0 : yaml.style) || 'list').onChange(async (value) => {
			yaml.style = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
			refresh(contentEl, yaml, plugin, defaultSettings, display, el, ctx, file, settingsTab, modal);
		}));
	...
	createOrReplaceSetting(contentEl, 'include-types', changedSection, (settingEl) => {
		var _a, _b;
		const setting = new import_obsidian40.Setting(settingEl);
	...
	createOrReplaceSetting(contentEl, 'file-tag', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Disable file tag').setDesc('Choose if the file tag should be shown after the file name').addToggle((toggle) => {
	...
	createOrReplaceSetting(contentEl, 'show-folder-notes', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Show folder notes').setDesc('Choose if folder notes (the note itself and not the folder name) should be shown in the overview').addToggle((toggle) => toggle.setValue(yaml.showFolderNotes).onChange(async (value) => {
			yaml.showFolderNotes = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
		}));
	});
	createOrReplaceSetting(contentEl, 'file-depth', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('File depth').setDesc('File & folder = +1 depth').addSlider((slider) => slider.setValue((yaml == null ? void 0 : yaml.depth) || 2).setLimits(1, 10, 1).setDynamicTooltip().onChange(async (value) => {
			yaml.depth = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
		}));
	});
	createOrReplaceSetting(contentEl, 'sort-files', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Sort files by').setDesc('Choose how the files should be sorted').addDropdown((dropdown) => dropdown.addOption('name', 'Name').addOption('created', 'Created').addOption('modified', 'Modified').setValue((yaml == null ? void 0 : yaml.sortBy) || 'name').onChange(async (value) => {
			yaml.sortBy = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
	...
	createOrReplaceSetting(contentEl, 'show-empty-folders', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Show folder names of folders that appear empty in the folder overview').setDesc("Show the names of folders that appear to have no files/folders in the folder overview. That's mostly the case when you set the file depth to 1.").addToggle((toggle) => {
	...
	createOrReplaceSetting(contentEl, 'show-empty-folders-only-first-level', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Only show empty folders which are on the first level of the folder overview').addToggle((toggle) => {
	...
	createOrReplaceSetting(contentEl, 'disable-collapse-icon', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Disable collapse icon for folder notes').setDesc('Remove the collapse icon next to the folder name for folder notes when they only contain the folder note itself').addToggle((toggle) => {
	...
	createOrReplaceSetting(contentEl, 'store-collapse-condition', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Store collapsed condition').setDesc('Choose if the collapsed condition should be stored until you restart Obsidian').addToggle((toggle) => toggle.setValue(yaml.storeFolderCondition).onChange(async (value) => {
			yaml.storeFolderCondition = value;
			updateSettings(contentEl, yaml, plugin, defaultSettings, el, ctx, file);
		}));
	});
	createOrReplaceSetting(contentEl, 'collapse-all-by-default', changedSection, (settingEl) => {
		new import_obsidian40.Setting(settingEl).setName('Collapse all in the tree by default').setDesc('Collapse every folder in the file explorer in the overview by default').addToggle((toggle) => {
...
// src/obsidian-folder-overview/src/modals/Settings.ts
var FolderOverviewSettings = class extends import_obsidian41.Modal {
...
var FileExplorerOverview = class {
	...
	async renderFileExplorer() {
		...
		newFolderElement.querySelectorAll('div.nav-folder-title').forEach((el) => {
			...
			if (!(folder2 instanceof import_obsidian42.TFolder))
				return;
		...
		if (tFolder instanceof import_obsidian42.TFolder) {
	...
	async addFiles(files, childrenElement, folderOverview, sourceFolderPath) {
		...
		const folders = sortedFiles.filter((child) => child instanceof import_obsidian42.TFolder);
		const otherFiles = sortedFiles.filter((child) => child instanceof import_obsidian42.TFile);
		...
		for (const child of otherFiles) {
			if (!(child instanceof import_obsidian42.TFile))
				continue;
	...
	async createFolderEL(plugin, child, folderOverview, childrenElement, sourceFolderPath) {
		...
		if (collapseIcon) {
			(0, import_obsidian42.setIcon)(collapseIcon, 'chevron-down');
			(_b = collapseIcon.querySelector('path')) == null ? void 0 : _b.setAttribute('d', 'M3 8L12 17L21 8');
		...
		if (yaml.allowDragAndDrop) {
			...
			fileTitle.addEventListener('dragover', (e) => {
				...
				if (draggable) {
					...
					(_c = (_b = fileElement.parentElement) == null ? void 0 : _b.parentElement) == null ? void 0 : _c.classList.add('is-being-dragged-over');
				}
			});
			fileTitle.addEventListener('dragleave', () => {
				var _a2, _b;
				(_b = (_a2 = fileElement.parentElement) == null ? void 0 : _a2.parentElement) == null ? void 0 : _b.classList.remove('is-being-dragged-over');
			});
			fileTitle.addEventListener('drop', (e) => {
				...
				if (draggable == null ? void 0 : draggable.file) {
					...
					(_c = (_b = fileElement.parentElement) == null ? void 0 : _b.parentElement) == null ? void 0 : _c.classList.remove('is-being-dragged-over');
				}
			});
		}
		fileTitle.onclick = () => {
...
function renderListOverview(plugin, ctx, root, yaml, pathBlacklist, folderOverview) {
	...
	if (!(tFolder instanceof import_obsidian43.TFolder)) {
		return;
	}
	...
	const folders = folderOverview.sortFiles(files.filter((f) => f instanceof import_obsidian43.TFolder));
	files = folderOverview.sortFiles(files.filter((f) => f instanceof import_obsidian43.TFile));
...
function addFolderList(plugin, list, pathBlacklist, folder, folderOverview) {
	...
	if (plugin instanceof FolderNotesPlugin) {
		const folderNote = getFolderNote(plugin, folder.path);
		if (folderNote instanceof import_obsidian43.TFile) {
...
async function goThroughFolders(plugin, list, folder, depth, sourceFolderPath, ctx, yaml, pathBlacklist, includeTypes2, disableFileTag, folderOverview) {
	...
	const files = folderOverview.sortFiles(allFiles.filter((file) => !(file instanceof import_obsidian43.TFolder) && file !== null));
	const folders = folderOverview.sortFiles(allFiles.filter((file) => file instanceof import_obsidian43.TFolder && file !== null));
	...
	folders.forEach((file) => {
		if (file instanceof import_obsidian43.TFolder) {
			if (yaml.includeTypes.includes('folder')) {
				...
				goThroughFolders(plugin, list, file, depth, sourceFolderPath, ctx, yaml, pathBlacklist, includeTypes2, disableFileTag, folderOverview);
			}
		}
	});
	files.forEach((file) => {
		if (file instanceof import_obsidian43.TFile) {
			if (yaml.includeTypes.includes('folder')) {
				...
				addFileList(plugin, list, pathBlacklist, file, includeTypes2, disableFileTag, folderOverview);
			}
		}
	});
}
...
var NewFolderNameModal = class extends import_obsidian44.Modal {
...
var FolderOverview = class {
	constructor(plugin, ctx, source, el, defaultSettings) {
		...
		this.plugin = plugin;
		this.emitter = new CustomEventEmitter2();
		let yaml = (0, import_obsidian45.parseYaml)(source);
		if (!yaml) {
			yaml = {};
		}
		const includeTypes2 = (yaml == null ? void 0 : yaml.includeTypes) || defaultSettings.includeTypes || ['folder', 'markdown'];
		...
		if (sourceFolder instanceof import_obsidian45.TFolder) {
	...
	async create(plugin, source, el, ctx) {
		...
		if (!sourceFolder && (sourceFolderPath !== '/' && sourceFolderPath !== '')) {
			return new import_obsidian45.Notice("Folder overview: Couldn't find the folder");
		}
		if (!sourceFolder && sourceFolderPath == '') {
			sourceFolderPath = '/';
		}
		if (!(sourceFolder instanceof import_obsidian45.TFolder) && sourceFolderPath !== '/') {
			return;
		}
		if (sourceFolderPath === '/') {
			const rootFiles = [];
			...
			files = rootFiles;
		} else if (sourceFolder instanceof import_obsidian45.TFolder) {
		...
		if (this.yaml.style === 'grid') {
			...
			files.forEach(async (file) => {
				...
				if (file instanceof import_obsidian45.TFile) {
					...
					link.href = file.path;
				} else if (file instanceof import_obsidian45.TFolder) {
	...
	sortFiles(files) {
		...
		files.sort((a, b) => {
			if (a instanceof import_obsidian45.TFolder && !(b instanceof import_obsidian45.TFolder)) {
				return -1;
			}
			if (!(a instanceof import_obsidian45.TFolder) && b instanceof import_obsidian45.TFolder) {
				return 1;
			...
			if (a instanceof import_obsidian45.TFile && b instanceof import_obsidian45.TFile) {
	...
	folderMenu(folder, e) {
		const plugin = this.plugin;
		const folderMenu = new import_obsidian45.Menu();
...
async function updateYaml(plugin, ctx, el, yaml) {
	...
	if (!(file instanceof import_obsidian45.TFile))
		return;
	let stringYaml = (0, import_obsidian45.stringifyYaml)(yaml);
...
async function getOverviews(plugin, file) {
	...
	for (const block of yamlBlocks) {
		const yaml = (0, import_obsidian45.parseYaml)(block.replace('```folder-overview\n', '').replace('```', ''));
		if (!yaml)
			continue;
		overviews.push(yaml);
	}
	...
	plugin.app.vault.process(file, (text) => {
		...
		for (const block of yamlBlocks) {
			...
			if (yaml.id === overviewId) {
				const stringYaml = (0, import_obsidian45.stringifyYaml)(newYaml);
...
function parseOverviewTitle(overview, plugin, folder) {
	...
	if ((folder == null ? void 0 : folder.path) === '/' && sourceFolderPath === '' || sourceFolderPath === '/') {
		...
		if (newSourceFolder instanceof import_obsidian45.TFolder) {
			return title.replace('{{folderName}}', newSourceFolder.name);
		}
	}
	return title;
}
var CustomMarkdownRenderChild = class extends import_obsidian45.MarkdownRenderChild {
...
var FolderOverviewView = class extends import_obsidian46.ItemView {
	...
	async display(contentEl, yaml, plugin, defaultSettings, display, el, ctx, file, settingsTab, modal, changedSection) {
		...
		if (!dropdown || changedSection === 'all' || changedSection === 'dropdown') {
			...
			const overviewSetting = new import_obsidian46.Setting(dropdown);
			overviewSetting.setName('Select overview').setClass('fn-select-overview-setting').addDropdown((cb) => {
				...
				if (activeFile) {
					...
					const options = overviews.reduce((acc, overview) => {
						let title = parseOverviewTitle(overview, plugin, activeFile.parent);
...
// src/obsidian-folder-overview/src/main.ts
var FolderOverviewPlugin = class extends import_obsidian47.Plugin {
	constructor() {
		super(...arguments);
		this.updateOverviewView = updateOverviewView;
		this.updateViewDropdown = updateViewDropdown;
	...
	async handleOverviewBlock(source, el, ctx) {
		const observer = new MutationObserver(() => {
			...
			if (editButton) {
				editButton.addEventListener('click', (e) => {
					...
					e.preventDefault();
					e.stopPropagation();
					new FolderOverviewSettings(this.app, this, (0, import_obsidian47.parseYaml)(source), ctx, el, this.settings).open();
				}, { capture: true });
			}
		});
		observer.observe(el, {
		...
		try {
			this.app.workspace.onLayoutReady(async () => {
				...
				await folderOverview.create(this, (0, import_obsidian47.parseYaml)(source), el, ctx);
				this.updateOverviewView(this);
			});
		} catch (e) {
			new import_obsidian47.Notice('Error creating folder overview (folder notes plugin) - check console for more details');
			console.error(e);
		}
	}
	async onunload() {
